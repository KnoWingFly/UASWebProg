            ...
            <div
                ...
                <div class="flex flex-col lg:flex-row gap-6 items-start">
                    <div>
                        ...
                        <p class="text-gray-400 mb-4 text-sm"> Updated: {{ $latestCategory->updated_at->format('Y-m-d') }}
                            @if($latestCategory->parent) | Parent: <span
                            class="text-blue-500">{{ $latestCategory->parent->name }}</span> @endif
                        </p>
    ...
    <div class="mb-8">
        ...
        <div id="categoriesGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            @foreach($categories as $category)
                @if($category->id !== optional($latestCategory)->id) <!-- Prevent null access -->
                    <div class="category-card opacity-0 bg-[#1a1a1a] border border-[#151515] rounded-lg shadow-lg hover:shadow-xl transition-all duration-300"
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <span class="bg-blue500 bg-opacity20 text-blue500 text-xs font-semibold px3 py1 rounded">
                                    {{ $category->learning_materials_count }} Materials </span>
                            </div>
                            <p class="text-gray400 mb4 text-sm"> Created: {{ $category->created_at->format('Y-m-d') }}
                                @if($category->parent) | Parent: <span class="text-blue500">{{ $category->parent->name }}</span>
                                @endif
                            </p>
                            <p class="text-sm text-gray400">{{ $category->description }}</p>
            ...
            @if($uncategorizedCount > 0)
                <div class="category-card opacity-0 bg-[#1a1a1a] border border-[#151515] rounded-lg shadow-lg hover:shadow-xl transition-all duration-300"
                    data-name="uncategorized" data-description="Materials without category" data-parent="" data-date="">
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-xl font-bold text-[#ff4d4d]">Uncategorized</h3>
                            <span class="bg-blue500 bg-opacity20 text-blue500 text-xs font-semibold px3 py1 rounded">
                                {{ $uncategorizedCount }} Materials </span>
                        </div>
...
<script>
    // JavaScript code for animations and filtering
    document.addEventListener('DOMContentLoaded', () => {
        // Initial GSAP animations
        gsap.to('#mainContainer', { opacity: 1, duration: 0.8, ease: 'power2.out' });
        gsap.to('#filterSection', { opacity: 1, y: 0, duration: 0.6, delay: 0.3, ease: 'power2.out' });
        gsap.to('#latestCategory', { opacity: 1, y: 0, duration: 0.6, delay: 0.5, ease: 'power2.out' });

        // Animate category cards
        gsap.utils.toArray('.category-card').forEach((card, index) => {
            gsap.to(card, { opacity: 1, y: 0, duration: 0.5, delay: 0.2 * index, ease: 'power2.out' });
        });
    });
    ...
    function filterCategories() {
        ...
        cards.forEach(card => {
            ...
            const matchesParentFilter = parentFilter === '' || (parentFilter === 'parent' && parentType === 'parent') || (parentFilter === 'sub' && parentType === 'sub') || (parentFilter === 'uncategorized' && parentType === '');
    ...
    function sortCards(cards, sortFilter) {
        ...
        cards.forEach((card, index) => {
            ...
            gsap.to(card, { opacity: 1, scale: 1, duration: 0.3, delay: index * 0.1 });
        });
    }
</script>
