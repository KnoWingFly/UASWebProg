...
<div class="container mx-auto mt-8">
    ...
    <div class="flex gap-4 mb-4">
        ...
        <form action="{{ route('admin.users.manage') }}" method="GET" class="flex gap-2">
            ...
            <button type="submit" class="px-4 py-2 bg-[#ff4d4d] text-white rounded">Search</button>
        </form>

        <!-- Filter Dropdown -->
        <select id="date_filter" name="date_filter"
            class="px-4 py-2 border rounded bg-[#1a1a1a] text-white">
...
<tbody>
    @forelse ($users as $user)
        <tr class="bg-white border-b hover:bg-gray-100 dark:bg-[#1a1a1a] dark:border-[#1a1a1a] dark:hover:bg-[#151515]">
            <td class="px-6 py-4">
    ...
    <div class="bg-[#1a1a1a] text-white p-6 rounded shadow-lg max-w-sm w-full">
        ...
        <form id="editForm" action="{{ route('admin.users.update', ':userId') }}" method="POST">
            ...
            <div class="flex justify-end space-x-2">
                <button type="submit" class="px-4 py-2 bg-[#FF4D4D] text-white rounded hover:bg-[#FF3D3D]">Save Changes</button>
