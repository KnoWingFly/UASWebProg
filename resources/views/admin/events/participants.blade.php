...
<div class="p-6 space-y-6">
    ...
    <div class="p-6 bg-gray-800 rounded-lg shadow-md text-gray-200">
        ...
        <p class="text-sm text-gray-300">
            Current Participants: <span class="font-medium">{{ $event->participants->count() }}</span>/<span class="font-medium">{{ $event->participant_limit }}</span>
        </p>
    </div>
                    ...
                    <tbody>
                        @foreach ($participants as $participant)
                            <tr class="border-b border-gray-700 hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4">{{ $participant->pivot->created_at->format('d M Y, h:i A') }}</td>
                                <td class="px-6 py-4">
                                ...
                                <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                                    <div class="relative bg-gray-800 rounded-lg shadow">
                                        <div class="p-6 text-center">
                                            ...
                                            <form action="{{ route('admin.events.removeParticipant', [$event, $participant]) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                                ...
                                                <button data-modal-hide="remove-modal-{{ $participant->id }}" 
                                                        type="button" 
