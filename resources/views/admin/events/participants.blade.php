...
@section('content')
<div class="p-6 space-y-6 bg-[#121212]">
    <!-- Header -->
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-semibold text-white">Participants for {{ $event->name }}</h1>
        <a href="{{ route('admin.events.index') }}"
            class="px-4 py-2 bg-blue-700 text-white font-medium rounded hover:bg-blue-800 transition-colors duration-200">
            Back to Events
        </a>
    </div>

    <!-- Event Details -->
<div class="p-6 bg-[#1A1A1A] rounded-lg shadow-md text-gray-200">
    <h2 class="text-xl font-semibold text-gray-100">Event Details</h2>
    <p class="mt-2">Name: <span class="font-medium text-gray-300">{{ $event->name }}</span></p>

    <!-- Registration Period -->
    <p class="text-gray-500 mt-2">Registration Start:</p>
    ...
    <p class="text-gray-500 mt-2">Registration End:</p>
    <p class="text-sm text-gray-300">
...
<!-- Participants Table -->
<div class="p-6 bg-[#1A1A1A] rounded-lg shadow-md">
    <h2 class="text-xl font-semibold text-gray-200">Participants List</h2>

    @if($participants->isEmpty())
        <p class="text-gray-500 mt-4">No participants have registered for this event yet.</p>
    @else
